...
$(function() {
	$("#limit").change(function() {
		location.href="./MemSelectAll.do?limit="+$(this).val();
	});
	if (${!empty limit}) {
...
<div class="container px-4 px-lg-5">
	...
	<c:choose>
		...
		<c:otherwise>
			<a href="./MemSelectAll.do?page=${page-1}&limit=${limit}">[이전]</a>&nbsp;
		</c:otherwise>
	</c:choose> 
	<c:forEach var="start" begin="${startpage}" end="${endpage}" step="1">
		<c:choose>
			...
			<c:otherwise>
				<a href="./MemSelectAll.do?page=${start}&limit=${limit}">[${start}]</a>&nbsp;
			</c:otherwise>
		</c:choose>
	</c:forEach> 
	<c:choose>
		...
		<c:otherwise>
			<a href="./MemSelectAll.do?page=${page+1}&limit=${limit}">[다음]</a>
		</c:otherwise>
	</c:choose>
	</p>
</div>
