...
$(function() {
	$("#topic").change(function() {
		location.href="./PostSearch.do?gameNumber=${gameDTO.gameNumber}&keyfield=${keyfield}&keyword=${keyword}&topic="+$(this).val()
	})
	if (${!empty topic}) {
...
<header class="bg-dark py-5">
	<div class="container px-4 px-lg-5 my-5">
		<div class="text-center text-white">
			...
			<c:choose>
				<c:when test="${keyfield == 'memId'}">
					<p class="lead fw-normal text-white-50 mb-0">작성자 검색 : 키워드 - ${keyword}</p>			
				</c:when>
				<c:when test="${keyfield == 'postTitle'}">
					<p class="lead fw-normal text-white-50 mb-0">제목 검색 : 키워드 - ${keyword}</p>			
				</c:when>
				<c:when test="${keyfield == 'postContent'}">
					<p class="lead fw-normal text-white-50 mb-0">내용 검색 : 키워드 - ${keyword}</p>			
				</c:when>
				<c:otherwise>
					<p class="lead fw-normal text-white-50 mb-0">검색 : 키워드 - ${keyword}</p>	
				</c:otherwise>
			</c:choose>
			</div>
	</div>
